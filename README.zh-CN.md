...
> 基于 Mercury 扩展的目标契约型 AI Agent Runtime，内置权限加固工具、运行 trace、技能资产、评估反馈和 SQLite 支持的 Second Brain 记忆。

[English](README.md) | 简体中文

Superagent 会记住重要信息，在执行有风险的操作前先请求确认，并且可以通过 CLI 或 Telegram 以 24/7 后台进程运行。它适合需要本地文件操作、命令执行、长期记忆、定时任务、多模型兜底和可追踪 Agent Runtime 的个人 AI 助手场景。
...
| `/runtime` | 查看最近一次 Agent Runtime trace |
| `/trace` | `/runtime` 的别名 |
| `/proposals` | 查看最近的数据型进化提案 |
| `/tools` | 列出已加载工具 |
| `/skills` | 列出已安装 Skill |
...
MIT。原始 Mercury 工作 © [Cosmic Stack](https://github.com/cosmicstack-labs)；Superagent 改造由 [btnalit](https://github.com/btnalit) 维护。

## 社区

- Issues：[btnalit/superagent](https://github.com/btnalit/superagent/issues)
