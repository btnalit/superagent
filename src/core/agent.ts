...
export class Agent {
  ...
  private async handleMessage(msg: ChannelMessage): Promise<void> {
    ...
    try {
      ...
      const runtimeRun = createAgentRuntimeRun(msg.content, {
        runId: `run_${Date.now().toString(36)}_${msg.id}`,
        channelType: msg.channelType,
        maxToolCalls: MAX_STEPS,
        permissions: this.capabilities.permissions.getManifest(),
      ...
      if (runtimeRun.contract.riskTier === 'R4_FORBIDDEN') {
        runtimeRun.transition('REJECTED');
        const finalText = 'I cannot help with requests that bypass permissions, delete user data, or exfiltrate secrets.';
        const evaluation = runtimeRun.evaluate(finalText);
        const evolutionFeedback = runtimeRun.createEvolutionFeedback(evaluation);
        ...
        this.episodic.record({
          type: 'message',
          summary: `Rejected forbidden request: ${msg.content.slice(0, 100)}`,
          channelType: msg.channelType,
          metadata: {
            agentRuntime: runtimeEvidence,
          },
        });
      ...
      runtimeRun.transition('EXECUTING');

      const systemPrompt = this.buildSystemPrompt(runtimeRun.renderPromptContext());
              ...
              onStepFinish: async ({ toolCalls, toolResults }) => {
                if (toolCalls && toolResults && toolCalls.length > 0) {
                  ...
                  for (let i = 0; i < toolCalls.length; i++) {
                    ...
                    runtimeRun.recordToolCall(tc.toolName, tc.input as Record<string, unknown>, failed);
                  }
      ...
      runtimeRun.transition('VERIFYING');
      const evaluation = runtimeRun.evaluate(finalText, {
        aborted: loopAbortController.signal.aborted,
      });
      runtimeRun.transition('LEARNING');
      const evolutionFeedback = runtimeRun.createEvolutionFeedback(evaluation);
      runtimeRun.transition('ARCHIVED');
      const runtimeEvidence = runtimeRun.toEvidence(evaluation, evolutionFeedback);
      this.lastRuntimeEvidence = runtimeEvidence;
      this.evolutionProposals.enqueue(evolutionFeedback.proposals);
  ...
  private buildSystemPrompt(runtimeContext?: string): string {
...
Always specify owner and repo parameters on GitHub tools. The user's GitHub username is ${this.config.github.username || 'not set'}.'`;
  ...
  private async handleChatCommand(content: string, channelType: string, channelId: string): Promise<boolean> {
    ...
    if (cmd === '/runtime' || cmd === '/trace') {
      await channel.send(formatRuntimeTrace(this.lastRuntimeEvidence), channelId);
      return true;
    }

    if (cmd === '/proposals') {
      const proposals = this.evolutionProposals.getRecent(10);
      if (proposals.length === 0) {
        await channel.send('No evolution proposals have been queued yet.', channelId);
        return true;
      }
      const lines = [
        `**${proposals.length} recent evolution proposal${proposals.length === 1 ? '' : 's'}:**`,
        '',
        ...proposals.map((proposal) => {
          const target = proposal.targetId ? `${proposal.targetType}:${proposal.targetId}` : proposal.targetType;
          return `• ${target} — ${proposal.reason} [${proposal.signals.join(', ')}]`;
        }),
      ];
      await channel.send(lines.join('\n'), channelId);
      return true;
    ...
    await channel.withMenu(async (select) => {
      while (true) {
        ...
        const action = await select('Mercury Commands', [
          ...
          { value: 'runtime', label: 'Runtime Trace' },
          { value: 'proposals', label: 'Evolution Proposals' },
        ...
        if (action === 'runtime') {
          await this.handleChatCommand('/runtime', 'cli', channelId);
          continue;
        }

        if (action === 'proposals') {
          await this.handleChatCommand('/proposals', 'cli', channelId);
          continue;
        }
